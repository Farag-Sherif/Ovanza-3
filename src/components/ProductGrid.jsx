...
export default function ProductGrid({ products }) {
  ...
  const handleMouseEnter = (productId) => {
    setImageHoverStates((prev) => ({ ...prev, [productId]: true }));
  };

  const handleMouseLeave = (productId) => {
    setImageHoverStates((prev) => ({ ...prev, [productId]: false }));
  };
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8">
      {products?.map((product, idx) => {
        const isHovered = imageHoverStates[product.id] || false;
        const productName =
          language === "ar"
        ...
        return (
          ...
          >
            ...
            >
              ...
              {/* Product Image */}
              <img
                key={`${product.id}-${isHovered ? 'hover' : 'normal'}`}
                src={displayImage}
                alt={productName}
